...
public class DependencyUtil {
    ...
    public static boolean validBytemanInstall() {
        ...
        if (!env.containsKey("BYTEMAN_HOME")) {
            System.out.println("BYTEMAN_HOME environment variable is not set.");
            return false;
        }

        // Get the value of the BYTEMAN_HOME environment variable
        String bytemanHome = env.get("BYTEMAN_HOME");

        // Check if the byteman.jar file exists in the BYTEMAN_HOME directory
        File bytemanJar = new File(bytemanHome + "/lib/byteman.jar");
